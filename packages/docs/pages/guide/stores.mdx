...
Configured in `amodal.json`. The runtime requires a PostgreSQL database — set `DATABASE_URL` in your `.env` file:
...
The store layer uses [Drizzle ORM](https://orm.drizzle.team) internally, so the schema is managed automatically — no migration scripts needed.
