...
- **File tools.** Eight built-in tools for navigating and editing the repo. Enabled via `fileTools: true` in `amodal.json`.
- **Path allowlist.** All file operations are scoped to the agent's config directories (`skills/`, `knowledge/`, `connections/`, `stores/`, `pages/`, `automations/`, `evals/`, `agents/`, `tools/`). Sensitive files (`.env`, `amodal.json`, `package.json`, `pnpm-lock.yaml`, `tsconfig.json`) are blocked.

## The eight file tools

The admin agent has tools grouped by purpose: **discovery** (finding what's there) and **read/write** (operating on files).
...
| Tool | What it does |
|------|--------------|
| **`read_repo_file`** | Read a single file. Supports `offset` and `limit` for line-based pagination of large files. |
| **`read_many_repo_files`** | Batched read of up to 20 files. Good for "review every SKILL.md." |
...
| **`write_repo_file`** | Create a new file or fully rewrite an existing one. Creates parent directories as needed. Prefer `edit_repo_file` for targeted changes. |
