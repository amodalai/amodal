...
| **shell_exec** | Run scripts in the configured sandbox environment. |
...
| **store_\*** | Auto-generated from [store definitions](/guide/stores). |

Built-in tools are marked with `isBuiltin: true` and `source: 'built-in'`. They appear in the agent's tool list automatically.
