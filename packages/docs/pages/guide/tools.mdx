...
| **dispatch_task** | always | Spawn a sub-agent with its own isolated state machine. Shares the parent's tools. See [Sub-Agents](/learn/architecture/agents). |
...
| **web_search** | `webTools` block configured | Grounded web search via Gemini Flash + Google Search. Returns a synthesized answer with cited source URLs. See [Web Tools](#web-tools). |
| **fetch_url** | `webTools` block configured | Fetch and extract the main content of a URL via Gemini urlContext, with a local fetch + Readability fallback for private networks. See [Web Tools](#web-tools). |
| **memory** | `memory.enabled: true` in config | Persistent memory with `add`, `remove`, `list`, and `search` actions. Entries persist across sessions and are injected into the system prompt. See [Memory](/guide/config#memory). |
