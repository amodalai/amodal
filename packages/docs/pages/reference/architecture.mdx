...
```mermaid
%%{init: {'theme':'base', 'themeVariables': {'primaryColor':'#DBEAFE','primaryBorderColor':'#1E40AF','primaryTextColor':'#1E293B','lineColor':'#475569','fontFamily':'Inter, system-ui, sans-serif','clusterBkg':'#F8FAFC','clusterBorder':'#CBD5E1'}}}%%
flowchart TD
    subgraph clients[Chat Interfaces]
        CLI[CLI<br/>amodal chat]
        React[React SDK<br/>@amodalai/react]
        Widget[Chat Widget]
    end
    Runtime[Runtime<br/>@amodalai/runtime<br/><i>HTTP server, SSE streaming, sessions,<br/>state machine loop, tools, stores</i>]
    subgraph providers[Providers via Vercel AI SDK]
        Anthropic
        OpenAI
...
- **store_write / store_batch / query_store**: CRUD against the configured store backend (PGLite or Postgres via Drizzle).
...
- **load_knowledge / propose_knowledge**: Reads a KB document on demand, or proposes an addition for human review.
- **present**: Emits a widget SSE event with structured data for the client to render inline.
- **Custom tools**: Compiled from each tool's `handler.ts` via esbuild, then executed with a scoped `ctx` containing `request`, `store`, `log`, and `env`.
- **MCP tools**: Proxied to the MCP server they were discovered from.
- **Admin file tools** (in admin sessions): `read_repo_file`, `write_repo_file`, `delete_repo_file` with path validation.
