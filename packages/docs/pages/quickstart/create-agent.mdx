...
When you run `amodal connect slack`, the CLI installs the `@amodalai/connection-slack` plugin and scaffolds a connection directory with everything the agent needs to understand and use the Slack API:

```
connections/slack/
├── spec.json       ← API endpoints, methods, parameters
├── access.json     ← auth config (token reference, scopes)
...
The `surface.md` and `entities.md` files are what make this different from a raw OpenAPI spec. They teach the agent *when* to use the API and *what the data means* — in plain English. You can edit these files to customize behavior. For example, adding a line to `rules.md` like "Never post to #general without explicit confirmation" will change how the agent behaves.

You will be prompted to provide your Slack Bot Token, which is stored encrypted in `access.json` (or read from an environment variable reference).
