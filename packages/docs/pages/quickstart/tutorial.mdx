...
Create a `.env` file (`amodal init` already gitignores it):
...
Using OpenAI or Google? Replace `ANTHROPIC_API_KEY` with `OPENAI_API_KEY` or `GOOGLE_API_KEY`.
...
Pages are React components that give your agent a custom UI. They use hooks to read from stores and display data.

Create the file `pages/research-dashboard.tsx`:

```tsx
// pages/research-dashboard.tsx
import { useStoreList } from '@amodalai/react';

export const page = {
  description: 'Research Notes',
  stores: ['notes'],
  automations: ['daily-hn-digest'],
  icon: 'book-open',
};

export default function ResearchDashboard() {
  const { documents, isLoading, error } = useStoreList('notes', {
    sort: '-saved_at',
    limit: 20,
  });
        ...
        {documents.map((doc) => (
          ...
          >
            <h2 className="font-semibold text-foreground">
              {doc.payload.topic as string}
            </h2>
            <p className="mt-1 text-sm text-muted-foreground">
              {doc.payload.summary as string}
            </p>
            {Array.isArray(doc.payload.sources) && (
              <div className="mt-2 flex flex-wrap gap-2">
                {(doc.payload.sources as string[]).map((src, i) => (
                  <a
                    key={i}
                    href={src}
                    target="_blank"
            ...
            <div className="mt-2 text-xs text-muted-foreground">
              {doc.payload.saved_at as string}
            </div>
          </div>
        ))}
      </div>
...
> **Note:** Pages live in your project directory alongside the rest of your config. The admin agent can't create `.tsx` files in `pages/` yet — this one you create by hand.

After saving, the dev server hot-reloads. You'll see **Research Notes** appear in the sidebar of the chat UI at `http://localhost:3847`. Click it to see your persisted research notes rendered in a card layout.

{/* TODO: screenshot of Research Notes page */}

The page reads from the `notes` store using `useStoreList` with descending sort by `saved_at`. Every time the agent saves a new research note (or the daily automation runs), the page updates automatically.
