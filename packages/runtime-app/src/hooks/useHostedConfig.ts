 ...
 * 1. Fetches config from /api/config (server resolves deploy from hostname)
 * 2. Sets up token acquisition based on auth mode
 *
 * Returns { appId, getToken, loading, error } for use with AmodalProvider.
 * For local dev (/api/config returns 404), returns appId='local' with no token.
 */
export function useHostedConfig(): HostedConfig {
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      try {
        // Fetch deploy config — server resolves the deploy from the hostname
        const res = await fetch('/api/config');
        if (!res.ok) {
          if (res.status === 404) {
            // Local dev — no hosted config available
            setLoading(false);
            return;
          }
