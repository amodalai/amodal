  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold tracking-tight text-gray-900 dark:text-zinc-200">{doc.key}</h1>
        <div className="flex items-center gap-2">
          <span className="text-xs text-gray-400 dark:text-zinc-500 bg-gray-100 dark:bg-zinc-800 px-2 py-0.5 rounded font-mono">v{String(doc.version)}</span>
          {meta.stale && (
      ...
      {/* Fields */}
      <div className="bg-white dark:bg-zinc-900/50 border border-gray-200 dark:border-zinc-800 rounded-xl divide-y divide-gray-100 dark:divide-zinc-800/50">
        {Object.entries(schema).map(([fieldName, fieldDef]) => (
          <div key={fieldName} className="px-5 py-3.5 flex">
            <div className="w-44 text-sm font-medium text-gray-500 dark:text-zinc-400 shrink-0 pt-0.5">
              {fieldName}
              {fieldDef.nullable && (
                <span className="text-[10px] ml-1 text-gray-300 dark:text-zinc-600">?</span>
              )}
            </div>
            <div className="text-sm flex-1 min-w-0 text-gray-900 dark:text-zinc-200">
      ...
      {/* Trace */}
      {hasTrace && meta.trace && (
        <details className="bg-white dark:bg-zinc-900/50 border border-gray-200 dark:border-zinc-800 rounded-xl group">
          <summary className="px-5 py-3.5 cursor-pointer text-sm font-medium text-gray-500 dark:text-zinc-400 hover:text-gray-700 dark:hover:text-zinc-300 transition-colors">
            Reasoning Trace
          </summary>
          <pre className="px-5 py-4 border-t border-gray-100 dark:border-zinc-800 text-xs whitespace-pre-wrap text-gray-600 dark:text-zinc-400 bg-gray-50/50 dark:bg-zinc-900/30 overflow-auto max-h-96 scrollbar-thin font-mono">
            {meta.trace}
          </pre>
        </details>
      )}
...
function MetaItem({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="flex items-center gap-2 text-sm">
      ...
      <span className="text-gray-700 dark:text-zinc-300 font-mono text-xs truncate">{value}</span>
    </div>
  );
}
