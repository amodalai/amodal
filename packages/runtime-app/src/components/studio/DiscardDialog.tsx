 ...
 *
 * Shown from the `DraftWorkspaceBar` when the user clicks Discard. Destructive
 * action — calls `discardAll()` which wipes every staged draft for the
 * current user. The bar's refetch-on-success path brings the count back to
 * zero and the bar collapses to the empty state.
 *
...
export interface DiscardDialogProps {
  /** Number of drafts about to be discarded — used in the warning text. */
  count: number;
...
export function DiscardDialog({ count, onConfirm, onCancel }: DiscardDialogProps) {
  ...
  // Close on Escape for accessibility. Disabled while the confirm call is
  // in flight so a mis-typed Escape doesn't strand the caller with a
  // half-completed discard.
  useEffect(() => {
  ...
  async function handleConfirm(): Promise<void> {
    ...
    try {
      await onConfirm();
      // Parent closes the dialog on success; we don't call onCancel here
      // to avoid double-close + stale-state races.
    } catch (err) {
