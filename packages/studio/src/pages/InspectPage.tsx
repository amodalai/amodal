...
export function InspectPage() {
  ...
  const isValid = Boolean(kind && name && VALID_KINDS.has(kind ?? ''));

  useEffect(() => {
    if (!kind || !name || !VALID_KINDS.has(kind)) return;
    ...
    fetch(
      `${runtimeUrl}/inspect/${encodeURIComponent(kind)}/` +
      ...
      .then((r) => {
        ...
        return r.json() as Promise<Omit<InspectDetail, 'kind'>>;
      })
      .then((data) => {
         
        setDetail({ ...data, kind: validKind } as InspectDetail);
      })
      .catch(() => setError(true));
  }, [runtimeUrl, kind, name, isValid]);

  if (!kind || !name || !isValid) {
    return <Navigate to="/" replace />;
  }

  if (error) return <AgentOffline page={`inspect/${kind}/${name}`} />;
