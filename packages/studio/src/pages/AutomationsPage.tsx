  ...
  const fetchAutomations = useCallback(() => {
    fetch(studioApiUrl('/api/automations'), { signal: AbortSignal.timeout(5_000) })
      .then((r) => {
        ...
        return r.json() as Promise<{ automations: Automation[] }>;
      })
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <Link
                    to={`${AUTOMATION_DETAIL_PATH}/${encodeURIComponent(auto.name)}`}
              ...
              <div className="flex items-center gap-2 ml-4 flex-shrink-0">
                <RunNowButton name={auto.name} onComplete={fetchAutomations} />
                <ToggleButton name={auto.name} enabled={auto.enabled} onComplete={fetchAutomations} />
              </div>
            </div>
          ))}
        </div>
