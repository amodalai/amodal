...
import { ProposeKnowledgeTool, ProposeKBUpdateTool } from './propose-knowledge.js';
...
import type { ProposeKnowledgeParams } from './propose-knowledge.js';
...
function makeParams(overrides: Partial<ProposeKnowledgeParams> = {}): ProposeKnowledgeParams {
  return {
    action: 'create',
    scope: 'application',
...
describe('ProposeKnowledgeTool', () => {
  ...
  it('has correct tool name', () => {
    const tool = new ProposeKnowledgeTool(config, messageBus);
    expect(ProposeKnowledgeTool.Name).toBe(PROPOSE_KNOWLEDGE_TOOL_NAME);
    expect(tool.name).toBe('propose_knowledge');
  });

  it('backward compat alias works', () => {
    expect(ProposeKBUpdateTool).toBe(ProposeKnowledgeTool);
  });
  ...
  it('definition schema includes working_memory in category enum', () => {
    const tool = new ProposeKnowledgeTool(config, messageBus);
    const schema = tool.getSchema();
    const paramsSchema = schema.parametersJsonSchema as Record<string, unknown>;
    const props = paramsSchema['properties'] as Record<string, Record<string, unknown>>;
    const categoryEnum = props['category']['enum'] as string[];
    expect(categoryEnum).toContain('working_memory');
  });

  it('createInvocation returns instance', () => {
    const tool = new ProposeKnowledgeTool(config, messageBus);
...
describe('ProposeKnowledgeInvocation', () => {
  let config: Config;
  let messageBus: MessageBus;
  ...
  function createInvocation(params: ProposeKnowledgeParams, cfg?: Config) {
    const tool = new ProposeKnowledgeTool(cfg ?? config, messageBus);
  ...
  it('execute() with auto_approved status shows applied immediately', async () => {
    const mockResponse = { id: 'prop-auto', status: 'auto_approved' };
    vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce(
      new Response(JSON.stringify(mockResponse), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
    ...
    const invocation = createInvocation(makeParams({ category: 'working_memory' }));
    const result = await invocation.execute(new AbortController().signal);

    expect(result.error).toBeUndefined();
    expect(result.llmContent).toContain('applied immediately');
    expect(result.llmContent).toContain('auto-approved');
  });

  it('execute() sends source: agent:chat in request body', async () => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce(
      new Response(JSON.stringify({ id: 'prop-src', status: 'pending' }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
      }),
    );
  ...
  it('accepts working_memory as valid category', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce(
      new Response(JSON.stringify({ id: 'prop-wm', status: 'auto_approved' }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
      }),
    );
    ...
    expect(result.error).toBeUndefined();
    expect(result.llmContent).toContain('prop-wm');
  });
});
