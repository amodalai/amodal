...
class ProposeKnowledgeInvocation extends BaseToolInvocation<
  ProposeKnowledgeParams,
  ToolResult
> {
  constructor(
  ...
  getDescription(): string {
    return `propose_knowledge [${this.params.scope}]: ${this.params.title}`;
  }
  ...
  async execute(_signal: AbortSignal): Promise<ToolResult> {
    ...
    try {
      ...
      // Return different messages based on auto-approve status
      if (result.status === 'auto_approved') {
        const successMessage =
          `Knowledge update applied immediately.\n` +
          `Proposal ID: ${result.id}\n` +
          `Scope: ${this.params.scope}\n` +
          `Title: ${this.params.title}\n` +
          `Status: auto-approved and applied`;
 ...
 */
export class ProposeKnowledgeTool extends BaseDeclarativeTool<
  ProposeKnowledgeParams,
  ToolResult
> {
  ...
  protected createInvocation(
    ...
    _toolName?: string,
    _toolDisplayName?: string,
  ): ToolInvocation<ProposeKnowledgeParams, ToolResult> {
    return new ProposeKnowledgeInvocation(
      this.config,
      params,
      messageBus,
      _toolName,
      _toolDisplayName ?? 'Propose Knowledge',
    );
  }
}

// Backward compatibility aliases
export { ProposeKnowledgeTool as ProposeKBUpdateTool };
export type { ProposeKnowledgeParams as ProposeKBUpdateParams };
