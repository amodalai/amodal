...
- **No ephemeral project state.** Code comments must not reference mini-project phases, workstreams, milestones, sprints, refactor names, or other PM labels — these go stale the moment the project ships or gets renamed. Red flags: "Phase 1", "Phase 2", "WS1"/"WS2", "Stage 1 of the X refactor", "for the vercel-shaped rollout", "added in Q3 push", "used by Phase 1 callers", "Roadmap 3.1", "(G9)", "gotcha G2". If the comment is real (a legacy table kept for backward-compat, a plain-text vs structured-content distinction), rewrite it around the **technical** reality — not the project timeline that produced it.
- **No "where this came from" lineage.** Comments describing a file's history rot immediately. Red flags: "Replaces gemini-cli-core's session lifecycle", "Ported from runtime-app's FooPage", "Extracted from the request tool", "Replaces the 150-line initialization in the old SessionManager", "Mirrors the upstream ToolDefinition", "These supplement the upstream X enum", "The old manual hack is no longer needed", "was previously inline", "replaces the previous file-based JSON implementation". Describe what the file **is** and **does** now. If a migration is in-flight and the comment is guarding the migration, that's fine — but once the old thing is gone, the reference to it must go too. Lineage belongs in `git log`/`git blame`, not in doc-comments.
- No references to the current task, PR number, ticket, or who-asked ("added for the X flow", "fix for issue #123", "per @alice's request", "amodal#146") — that belongs in the PR description, not the code.
